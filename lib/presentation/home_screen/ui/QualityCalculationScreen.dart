...
class Qualitycalculationscreen extends ConsumerStatefulWidget {
  const Qualitycalculationscreen({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _QualitycalculationscreenState();
}

class _QualitycalculationscreenState
    extends ConsumerState<Qualitycalculationscreen> {
  ...
  var textControllerListProvider = StateProvider<List<TextEditingController>>(
    (ref) => [],
  );
  var qualityParamList = StateProvider<List<QualityParamDatum>?>((ref) => []);
  var qvPrice = StateProvider<String>((ref) => '0');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context)!.calculateQuality),
      ),
        ...
        child: Form(
          ...
          child: Column(
            children: [
                  ...
                  .when(
                    data: (data) {
                      return DropdownSearch<StatesDatum?>(
                        compareFn:
                            (states, states2) => states?.id == states2?.id,
                        ...
                        return DropdownSearch<DistrictDatum?>(
                          ...
                          popupProps: PopupPropsMultiSelection.menu(
                            ...
                            title: Padding(
                              padding: Pad(all: 10),
                              child: Text(
                                AppLocalizations.of(context)!.selectDistrict3,
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontSize: Adaptive.sp(16),
                                  fontWeight: FontWeight.w700,
              ...
              SizedBox(height: 10),
              if (ref.watch(statesProvider) != null &&
                  ref.watch(districtProvider) != null)
                ref
                    .watch(warehouseDataProvider)
                    .when(
                      data:
                          (data) => DropdownSearch<Commodite?>(
                            compareFn: (comm1, comm2) => comm1?.id == comm2?.id,
                            popupProps: PopupPropsMultiSelection.menu(
                              constraints: BoxConstraints(
                                maxHeight: Get.height / 1.5,
                              ),
              ...
              if (ref.watch(statesProvider) != null &&
                  ref.watch(districtProvider) != null &&
                  ref.watch(commodityDropDownProvider) != null)
                Column(
                  children: [
                    Container(
                      padding: Pad(all: 10),
                      ...
                      child: IntrinsicHeight(
                        child: Row(
                          children: [
                            Expanded(
                              child: Text(
                                AppLocalizations.of(context)!.paramarters2,
                                textAlign: TextAlign.center,
                                style: TextStyle(
                            ...
                            Expanded(
                              child: Text(
                                AppLocalizations.of(context)!.max3,
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  color: Colors.white,
                            ...
                            Expanded(
                              child: Text(
                                AppLocalizations.of(context)!.min3,
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  color: Colors.white,
                            ...
                            Expanded(
                              child: Text(
                                AppLocalizations.of(context)!.enter2,
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  color: Colors.white,
                    ...
                    ref
                        .watch(
                          qualityParametersProvider(
                            ...
                            districtId: "${ref.watch(districtProvider)?.id}",
                            commodityId:
                            ...
                            return ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                Future.delayed(Duration.zero).then((_) {
                                  ref
                                      .watch(textControllerListProvider)
                                      .add(TextEditingController());
                                  ref.watch(qualityParamList.notifier).state =
                                      data.data;
                    ...
                    ElevarmTextInputField(
                      label: AppLocalizations.of(context)!.enterAmount2,
                      ...
                      validator: (value) {
                        if (value == null || value.isEmpty) {
                          return AppLocalizations.of(context)!.inputValidValue4;
                        }
                        if (value.toString() == "0") {
                          return AppLocalizations.of(context)!.inputValidValue4;
                        }
                    ...
                    ElevarmPrimaryButton.text(
                      text: AppLocalizations.of(context)!.calculatePrice2,
                      onPressed: () {
                        List<Map<dynamic, dynamic>> listOfParams = [];
                        if (formkey.currentState!.validate()) {
                          for (
                            var i = 0;
                            i < ref.watch(qualityParamList)!.length;
                            i++
                          ) {
                            ref.watch(qualityParamList)![i].value =
                                ref.watch(textControllerListProvider)[i].text;
                          }

                          for (var e in ref.watch(qualityParamList)!) {
                            listOfParams.add(e.toJson());
                          }
                          Map<String, dynamic> data = {
                            'state_code': "${ref.watch(statesProvider)?.code}",
                            "district_id": "${ref.watch(districtProvider)?.id}",
                            'commodity_id':
                                "${ref.watch(commodityDropDownProvider)?.id}",
                          ...
                          ref
                              ...
                              .then((value) {
                                if (value['status'].toString() == "1") {
                                  ref.watch(qvPrice.notifier).state =
                                      value['final_qv'].toString();
                                  print(value['data']);
                                }
                              });
                        }
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 20),
                        Text(
                          "Final Price : ${num.parse(priceController.text.isEmpty ? '0' : priceController.text) - num.parse(ref.watch(qvPrice))}",
                          style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: Adaptive.sp(17),
                          ),
