...
class Sbtmysell extends ConsumerStatefulWidget {
  const Sbtmysell({super.key});

  @override
  ConsumerState<Sbtmysell> createState() => _SbtmysellState();
}

class _SbtmysellState extends ConsumerState<Sbtmysell> {
  @override
            ...
            : ref.watch(sbtContactNoteProvider).when(
                  data: (data) {
                    var sellerList = data.data
                        ?.where(
                            (element) => element.type.toLowerCase() == "seller")
                        .toList();

                    if ((sellerList ?? []).isEmpty) return noStockData(context);
                      ...
                      children: [
                        ...
                        ListView.builder(
                            ...
                            itemBuilder: (context, index) => _buildItemRow(
                                context, sellerList![index], index))
                      ],
                    );
                  },
        ...
        children: [
          ElevatedButton(
            style: AppStyle.buttonStyle.copyWith(
              ...
              backgroundColor: WidgetStatePropertyAll(ColorConstant.maingreen),
            ),
  ...
  Future<void> _handleDownload(BuildContext context, dynamic item) async {
    try {
      final contractNote = await ref.watch(sellerContractNoteProvider(
              orderId: item.orderId.toString(), type: "Seller")
          .future);
