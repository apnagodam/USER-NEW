...
class _DeliveryMarkingState extends ConsumerState<DeliveryMarking> {
  ...
  @override
  Widget build(BuildContext context) {
// ✅ KEEP THIS — auto-refresh lists when provider updates
  ref.listen(matchedOrdersProvider, (previous, next) {
    next.whenData((value) {
      final buyOrders = value.data
              ?.where((element) => element.type.toString().toLowerCase() == "buy")
              .toList() ?? [];
      ref.read(buyOrdersListProvider.notifier).state = buyOrders;

      final sellerOrders = value.data
              ?.where((element) => element.type.toString().toLowerCase() == "sell")
              .toList() ?? [];
